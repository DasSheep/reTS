...
step-by-step instructions before those steps exist would be misleading, so this
page records availability instead of speculative guidance.
:::
...
- This page will change from a status page to a guide only after a public build
  and its asset-discovery flow are verified.

## Release constraint

reTS will not bundle original Command & Conquer game assets. A future player
release will require legally owned game data from the supported titles, with
exact supported inputs documented alongside that release rather than guessed in
advance.
