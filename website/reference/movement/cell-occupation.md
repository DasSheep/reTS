...
Tiberian Sun's shared occupancy layer carries **no** owner stamp of this kind. The whole stamp-and-reset mechanism above is a Red Alert 2 / Yuri's Revenge feature.

Tiberian Sun does have a per-cell owner field, but it is a **separate cell field**, unrelated to the occupancy layers, and nothing in the mark/unmark path touches it. Two independent writers have been confirmed: the flag-placement routine, which stores the house passed to it, and the wall-owner scan, which walks the house list and stamps the winning house onto a wall cell. Notably that second writer leaves the field **completely untouched** on any cell that is not a wall — both of its early exits jump past the store rather than clearing it — so a non-wall cell's owner field retains whatever a previous writer left there.
